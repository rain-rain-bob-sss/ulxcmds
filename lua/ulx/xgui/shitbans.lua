--Bans module for ULX GUI -- by Stickly Man! -- and edited by nil
--Manages banned users and shows ban details

xgui.prepareDataType( "shitbans" )

local xsbans = xlib.makepanel{ parent=xgui.null }

xsbans.banlist = xlib.makelistview{ x=5, y=30, w=572, h=310, multiselect=false, parent=xsbans }
	xsbans.banlist:AddColumn( "Name/SteamID" )
	xsbans.banlist:AddColumn( "Banned By" )
	xsbans.banlist:AddColumn( "Unban Date" )
	xsbans.banlist:AddColumn( "Reason" )
xsbans.banlist.DoDoubleClick = function( self, LineID, line )
	xsbans.ShowBanDetailsWindow( xgui.data.bans.cache[LineID] )
end
xsbans.banlist.OnRowRightClick = function( self, LineID, line )
...
xsbans.banlist.SortByColumn = function( self, ColumnID, Desc )
	...
	xsbans.sortbox:ChooseOptionID( index )
end

local searchFilter = ""
xsbans.searchbox = xlib.maketextbox{ x=5, y=6, w=175, text="Search...", selectall=true, parent=xsbans }
local txtCol = xsbans.searchbox:GetTextColor() or Color( 0, 0, 0, 255 )
xsbans.searchbox:SetTextColor( Color( txtCol.r, txtCol.g, txtCol.b, 196 ) ) -- Set initial color
xsbans.searchbox.OnChange = function( pnl )
	if pnl:GetText() == "" then
		pnl:SetText( "Search..." )
		pnl:SelectAll()
...
xsbans.searchbox.OnLoseFocus = function( pnl )
...
local sortMode = 0
local sortAsc = false
xsbans.sortbox = xlib.makecombobox{ x=185, y=6, w=150, text="Sort: Date (Desc.)", choices={ "Date", "Name", "Steam ID", "Admin", "Reason", "Unban Date", "Ban Length" }, parent=xsbans }
function xsbans.sortbox:OnSelect( i, v )
...
local hidePerma = 0
xlib.makebutton{ x=355, y=6, w=95, label="Permabans: Show", parent=xsbans }.DoClick = function( self )
...
xlib.makebutton{ x=455, y=6, w=95, label="Incomplete: Show", parent=xsbans, tooltip="Filters bans that are loaded by ULib, but do not have any metadata associated with them." }.DoClick = function( self )
...
local function banUserList( doFreeze )
	...
	menu:AddSpacer()
	if LocalPlayer():query("ulx shitbanid") then menu:AddOption( "Ban by STEAMID...", function() xgui.ShowBanWindow() end ) end
	menu:Open()
end

xlib.makebutton{ x=5, y=340, w=70, label="Ban...", parent=xsbans }.DoClick = function() banUserList( false ) end
xsbans.btnFreezeBan = xlib.makebutton{ x=80, y=340, w=95, label="Freeze Ban...", parent=xsbans }
xsbans.btnFreezeBan.DoClick = function() banUserList( true ) end

xsbans.infoLabel = xlib.makelabel{ x=204, y=344, label="Right-click on a ban for more options", parent=xsbans }


xsbans.resultCount = xlib.makelabel{ y=344, parent=xsbans }
function xsbans.setResultCount( count )
	local pnl = xsbans.resultCount
	...
	local ix, iy = xsbans.infoLabel:GetPos()
	xsbans.infoLabel:SetPos( ( 130 - width ) / 2 + 175, y )
end

local numPages = 1
local pageNumber = 1
xsbans.pgleft = xlib.makebutton{ x=480, y=340, w=20, icon="icon16/arrow_left.png", centericon=true, disabled=true, parent=xsbans }
xsbans.pgleft.DoClick = function()
	xsbans.setPage( pageNumber - 1 )
	xsbans.retrieveBans()
end
xsbans.pageSelector = xlib.makecombobox{ x=500, y=340, w=57, text="1", enableinput=true, parent=xsbans }
function xsbans.pageSelector:OnSelect( index )
	xsbans.setPage( index )
	xsbans.retrieveBans()
end
function xsbans.pageSelector.TextEntry:OnEnter()
	pg = math.Clamp( tonumber( self:GetValue() ) or 1, 1, numPages )
	xsbans.setPage( pg )
	xsbans.retrieveBans()
end
xsbans.pgright = xlib.makebutton{ x=557, y=340, w=20, icon="icon16/arrow_right.png", centericon=true, disabled=true, parent=xsbans }
xsbans.pgright.DoClick = function()
	xsbans.setPage( pageNumber + 1 )
	xsbans.retrieveBans()
end

xsbans.setPage = function( newPage )
	pageNumber = newPage
	xsbans.pgleft:SetDisabled( pageNumber <= 1 )
	xsbans.pgright:SetDisabled( pageNumber >= numPages )
	xsbans.pageSelector.TextEntry:SetText( pageNumber )
end


function xsbans.RemoveBan( ID, bandata )
	...
	Derma_Query( "Are you sure you would like to unban " .. tempstr .. " - " .. ID .. "?", "XGUI WARNING",
		"Remove",	function()
						RunConsoleCommand( "ulx", "unshitban", ID )
						xsbans.RemoveBanDetailsWindow( ID )
					end,
		"Cancel", 	function() end )
end

xsbans.openWindows = {}
function xsbans.RemoveBanDetailsWindow( ID )
	if xsbans.openWindows[ID] then
		xsbans.openWindows[ID]:Remove()
		xsbans.openWindows[ID] = nil
	end
end

function xsbans.ShowBanDetailsWindow( bandata )
	local wx, wy

	if not bandata then return end

	if xsbans.openWindows[bandata.steamID] then
		wx, wy = xsbans.openWindows[bandata.steamID]:GetPos()
		xsbans.openWindows[bandata.steamID]:Remove()
	end
	xsbans.openWindows[bandata.steamID] = xlib.makeframe{ label="Ban Details", x=wx, y=wy, w=285, h=295, skin=xgui.settings.skin }

	local panel = xsbans.openWindows[bandata.steamID]
	...
	xlib.makebutton{ x=99, y=265, w=89, label="Unban", parent=panel }.DoClick = function()
		xsbans.RemoveBan( panel.data.steamID, panel.data )
	end

	xlib.makebutton{ x=192, y=265, w=88, label="Close", parent=panel }.DoClick = function()
		xsbans.RemoveBanDetailsWindow( panel.data.steamID )
	end
	...
	xlib.makebutton{ x=45, y=150, w=75, label=( isUpdate and "Update" or "Ban!" ), parent=xgui_banwindow }.DoClick = function()
		if isUpdate then
			local function performUpdate(btime)
				RunConsoleCommand( "xgui", "updateShitBan", steamID:GetValue(), btime, reason:GetValue(), name:GetValue() )
				xgui_banwindow:Remove()
			end
			...
			if btime ~= 0 and bandata and btime * 60 + bandata.time < os.time() then
				Derma_Query( "WARNING! The new ban time you have specified will cause this ban to expire.\nThe minimum time required in order to change the ban length successfully is "
					...
					"Expire Ban", function()
						performUpdate(btime)
						xsbans.RemoveBanDetailsWindow( bandata.steamID )
					end,
					"Cancel", function() end )
			else
				performUpdate(btime)
		...
		if ULib.isValidSteamID( steamID:GetValue() ) then
			...
			if not isOnline then
				if name:GetValue() == "" then
					RunConsoleCommand( "ulx", "shitbanid", steamID:GetValue(), banpanel:GetValue(), reason:GetValue() )
				else
					RunConsoleCommand( "xgui", "updateShitBan", steamID:GetValue(), banpanel:GetMinutes(), reason:GetValue(), ( name:GetValue() ~= "" and name:GetValue() or nil ) )
				end
			else
				RunConsoleCommand( "ulx", "shitban", "$" .. ULib.getUniqueIDForPlayer( isOnline ), banpanel:GetValue(), reason:GetValue() )
			end
			xgui_banwindow:Remove()
		else
			local ply, message = ULib.getUser( name:GetValue() )
			if ply then
				RunConsoleCommand( "ulx", "shitban", "$" .. ULib.getUniqueIDForPlayer( ply ), banpanel:GetValue(), reason:GetValue() )
				xgui_banwindow:Remove()
				return
			end
...
xgui.hookEvent( "shitbans", "process", xsbans.bansRefreshed, "bansRefresh" )

function xsbans.banPageRecieved( data )
	xgui.data.bans.cache = data
	xsbans.clearbans()
	xsbans.populateBans()
end
xgui.hookEvent( "shitbans", "data", xsbans.banPageRecieved, "bansGotPage" )

function xsbans.checkCache()
	if xgui.data.bans.cache and xgui.data.bans.count ~= 0 and table.Count(xgui.data.bans.cache) == 0 then
		xsbans.retrieveBans()
	end
end
xgui.hookEvent( "onOpen", nil, xsbans.checkCache, "bansCheckCache" )

function xsbans.clearbans()
	xsbans.banlist:Clear()
end

function xsbans.retrieveBans()
	RunConsoleCommand( "xgui", "getshitbans",
		sortMode,			-- Sort Type
...
function xsbans.populateBans()
	...
	xsbans.setResultCount( count )
	xsbans.pageSelector:SetDisabled( numPages == 1 )
	xsbans.pageSelector:Clear()
	for i=1, numPages do
		xsbans.pageSelector:AddChoice(i)
	end
	xsbans.setPage( math.Clamp( pageNumber, 1, numPages ) )

	cache.count = nil

	for _, baninfo in pairs( cache ) do
		xsbans.banlist:AddLine( baninfo.name or baninfo.steamID,
...
xsbans.populateBans() --For autorefresh

function xsbans.xsban( ply, cmd, args, dofreeze )
...
ULib.cmds.addCommandClient( "xgui xsban", xsbans.xsban )

function xsbans.fsban( ply, cmd, args )
	xsbans.xban( ply, cmd, args, true )
end
ULib.cmds.addCommandClient( "xgui fsban", xsbans.fsban )

function xsbans.UCLChanged()
	xsbans.btnFreezeBan:SetDisabled( not LocalPlayer():query("ulx freeze") )
end
hook.Add( "UCLChanged", "xgui_RefreshBansMenu", xsbans.UCLChanged )

xgui.addModule( "Shitbans", xsbans, "icon16/camera_delete.png", "xgui_manageshitbans" )
